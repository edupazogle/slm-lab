#!/usr/bin/env python3
"""
Regex baseline for E1 anonymisation detection.
Uses patterns from slm/app/second-look/index.html to detect PII in text.
DEBUG VERSION - adds debug prints
"""
...
# PERSON detection constants from second-look/index.html
...
# Extended first name list with common English, French, Spanish, German and Italian names
...
# Extended NOT_NAME list with common words that are not names
...
def name_candidates(text: str) -> List[Dict]:
...
def extract_patterns(text: str) -> List[Dict]:
    ...
    # Patterns from slm/app/second-look/index.html lines 1032-1046
    patterns = [
        # EMAIL
        ...
        # NIR (French social security number) - flexible separators (space or hyphen)
        (r'\b[12][\s-]?\d{2}[\s-]?(?:0[1-9]|1[0-2])[\s-]?(?:\d{2}|2[AB])[\s-]?\d{3}[\s-]?\d{3}(?:[\s-]?\d{2})?\b', 'NIR', 0, None),
    ...
    for pattern, label, validator_group in compiled_patterns:
        for match in pattern.finditer(text):
            ...
            if validator and not validator(matched_text.replace(' ', '')):
                continue
    ...
    entities.extend(name_candidates(text))
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python regex_baseline_debug.py <input_jsonl> <output_jsonl>")
        sys.exit(1)
    ...
    with open(input_file, 'r', encoding='utf-8') as f_in, \
        ...
        for i, line in enumerate(f_in):
            try:
                ...
                # Write empty record to maintain line count
                out_record = {
                    'id': f'error_{i}',
                    'lang': 'unknown',
                    'text': '',
