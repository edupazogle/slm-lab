...
def load_lm_scores():
    """Load LM scores from cache file."""
    lm_scores_path = os.path.join(DATA_DIR, "lm_scores.jsonl")
    lm_scores = {}

    with open(lm_scores_path, 'r', encoding='utf-8') as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            try:
                ...
                score = entry['lm_score']
                lm_scores[window_id] = score
            except (json.JSONDecodeError, KeyError) as e:
                print(f"Error parsing LM score line: {e}", file=sys.stderr)
                continue

    return lm_scores

def load_dataset_with_ids(split):
    """Load dataset from JSONL file and return window IDs for LM score mapping."""
    ...
    with open(dataset_path, 'r', encoding='utf-8') as f:
        for line_num, line in enumerate(f):
            ...
            try:
                ...
                ids.append(f"{split}_{line_num}")  # Match the ID format from lm_feature.py
            except (json.JSONDecodeError, KeyError) as e:
...
def extract_features_zlib_regex_lm(text, lm_score):
    """
    Extract features for baseline (2):
    - zlib compression ratio + regex features (from baseline 1)
    - LM score
    """
    # Get baseline 1 features
    features = extract_features_zlib_regex(text)
    # Add LM score as the last feature
    features.append(lm_score)
    return features
...
def evaluate_baseline_2(texts, labels, ids):
    """Evaluate baseline (2): zlib + regex features + LM score with logistic regression."""
    # Load LM scores
    lm_scores = load_lm_scores()

    # Extract features
    features_list = []
    for i, text in enumerate(texts):
        # Get LM score for this window
        window_id = ids[i]
        lm_score = lm_scores.get(window_id, 0.0)  # Default to 0.0 if not found

        # Extract features (zlib + regex + regex + LM)
        features = extract_features_zlib_regex_lm(text, lm_score)
        features_list.append(features)
...
def main():
    ...
    # Evaluate baseline 2 on train and test (zlib + regex + LM)
    print("\n=== Baseline 2: zlib + regex + LM features ===")
    # Need to get IDs for train and test
    _, _, train_ids = load_dataset_with_ids('train')
    _, _, test_ids = load_dataset_with_ids('test')
    train_metrics_2 = evaluate_baseline_2(train_texts, train_labels, train_ids)
    test_metrics_2 = evaluate_baseline_2(test_texts, test_labels, test_ids)

    print("Train:")
    print(f"  Accuracy: {train_metrics_2['accuracy']:.4f}")
    print(f"  Precision: {train_metrics_2['precision']:.4f}")
    print(f"  Recall: {train_metrics_2['recall']:.4f}")
    print(f"  F1: {train_metrics_2['f1']:.4f}")
    ...
    print(f"  F1: {test_metrics_2['f1']:.4f}")
    ...
    results = {
        ...
        'baseline_2': {
            'train': {k: v for k, v in train_metrics_2.items() if k not in ['predictions', 'model', 'scaler']},
            'test': {k: v for k, v in test_metrics_2.items() if k not in ['predictions', 'model', 'scaler']}
        }
    }
