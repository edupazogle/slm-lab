...
### Baseline 2 Performance (Test Set) - zlib + regex + LM features
From `results/baselines_results.json`:
| Metric | Value |
|--------|-------|
| Accuracy | 0.9658 |
...
| Metric | Value |
|--------|-------|
| Garble recall overall | 0.9008 |
| Garble recall on Latin-script slice | 0.8922 |
| False alarms per 8 agent-hours | 3.5726 |
| Pane-state accuracy | 0.9658 |
| Waiting-permission recall | 0.0000 |
| CPU ms per classification | 659.8 |
...
Actual results for Baseline 1 (zlib + regex):
...
Actual results for Baseline 2 (zlib + regex + LM):
- Garble recall overall: 0.9008 (**FAILS** - requires ≥ 0.95)
- Garble recall on Latin-script slice: 0.8922 (**FAILS** - requires ≥ 0.90)
- False alarms per 8 agent-hours: 3.5726 (**FAILS** - requires ≤ 1)
- Pane-state accuracy: 0.9658 (**PASSES** - requires ≥ 0.90)
- Waiting-permission recall: 0.0000 (**FAILS** - requires ≥ 0.95)
- CPU per poll: 659.8 ms (**PASSES** - requires ≤ 1000 ms)

### Kill Rule Evaluation
According to the experiment plan in E11-step-3.md:
> **Decision by plan's kill rule: Baseline 2 is kept only if it beats Baseline 1 by MORE than 2 points on garble recall without raising false alarms per 8 agent-hours above 1, AND stays ≤ 1 s CPU per poll**

Comparison:
- Baseline 1 garble recall: 0.9160 (91.60%)
- Baseline 2 garble recall: 0.9008 (90.08%)
- Difference: -0.0152 (-1.52 points) - Baseline 2 performed WORSE than Baseline 1 by 1.52 points
- False alarms per 8 agent-hours: Baseline 1 = 3.6446, Baseline 2 = 3.5726 (both above 1)
- CPU per poll: Baseline 1 = 0.50 ms, Baseline 2 = 659.8 ms (both below 1000 ms)

**Decision: Baseline 2 is NOT kept.** It failed the kill rule because:
1. It did NOT beat Baseline 1 by more than 2 points on garble recall (it performed worse by 1.52 points)
2. Both baselines have false alarms per 8 agent-hours above 1 (3.6446 and 3.5726 respectively)
3. Although both stay ≤ 1 s CPU per poll, the first two conditions are not met

Since Baseline 2 did not beat Baseline 1 by more than 2 points on garble recall, per the kill rule we keep Baseline 1 and drop the LM feature approach.
...
1. **Did not implement step 2 (add small-LM perplexity feature) as a kept baseline** - I did implement and evaluate the small-LM perplexity feature as Baseline 2 (zlib + regex + LM score). However, Baseline 2 did not beat Baseline 1 by more than 2 points on garble recall (it performed worse by 1.52 points), so per the kill rule in E11-step-3.md, Baseline 2 is not kept and we revert to Baseline 1.
