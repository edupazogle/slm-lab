...
|---|---|---|---|
| E11 watchdog | measured, pass bar **missed** | 1,307 windows from 64 real jobs + synthetic garble; zlib+regex+logistic: garble recall 0.916 (Latin slice 0.909), false alarms 3.64 / 8 agent-hours, pane-state accuracy 0.970, CPU 0.5 ms (`e11/results/summary.json`); zlib+regex+LM: garble recall 0.901 (Latin slice 0.892), false alarms 3.57 / 8 agent-hours, pane-state accuracy 0.966, CPU 659.8 ms | waiting-permission recall reads 0.0 (no such windows in the data); the `label` field is written as `is_synthetic` in the rows; small-LM feature tried but did not beat baseline by >2 points on garble recall |
